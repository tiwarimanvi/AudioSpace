...
const Polling = () => {
  ...
  useEffect(() => {
    ...
    optionRefs.current.forEach((ref, index) => {
      if (ref) {
        ref.addEventListener('keydown', (event) => handleKeyPress(event, index));
      }
    });
    ...
    return () => {
      optionRefs.current.forEach((ref, index) => {
        if (ref) {
          ref.removeEventListener('keydown', (event) => handleKeyPress(event, index));
        }
      });
    };
  }, [options]);

  // Effect to focus on the current option input field after typing
  useEffect(() => {
    if (optionRefs.current.length > 0) {
      const lastOptionIndex = optionRefs.current.length - 1;
            ...
            {options.map((option, index) => (
              <div key={nanoid()} className="flex items-center mb-2">
                <input
                  ...
                  maxLength={50}
                  value={option}
                  ref={el => optionRefs.current[index] = el} // Ref for input
